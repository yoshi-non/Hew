...
//qa__questionからの質問情報を取得する(ベストアンサーが回答にあるものは取得しない)
$questions = $db->query('
  SELECT DISTINCT(questions.id), questions.body, questions.created_at, members.name, members.picture
  FROM questions , answers , members
  WHERE questions.member_id = members.id 
  AND questions.id
  NOT IN(
    SELECT DISTINCT(questions.id)
    FROM questions, answers
    WHERE questions.id = answers.question_id
    AND answers.is_best_answer = 1
  )
  OR questions.id
  NOT IN(
    SELECT DISTINCT(questions.id) 
    ...
    <div class="quizBasic__main">
      ...
      <div id="wrap">
        ...
        <div class="qa__box">
          <div class="qa__nav">
            <ul>
              <li class="qa__open"><a href="qa__yet.php">回答受付中</a></li>
              <li class="qa__close"><a href="qa__fin.php">解決済</a></li>
              <li class="qa__close"><a href="qa__all.php">すべて</a></li>
            </ul>
          </div>

          <?php foreach($questions as $question): ?>
          <div class="qa__text__box">
            <a class="qa__text" href="qa__answer.php?id=<?php echo htmlspecialchars($question['id'], ENT_QUOTES); ?>" style="text-decoration: none;">
