...
if(!empty($_POST)){
  if(!empty($_POST['body'])){
    ...
    header("Location: qa__answer.php?id=" .$question['id']);
    exit();
    }
  }
...
//ベストアンサーに選択した回答のある質問をis_best_answer=1にする
// $best_question = $db->prepare('UPDATE questions SET is_best_answer=1 WHERE id=?');
// $best_question->execute(array($_POST['id']));
      ...
      <div id="wrap">
        ...
        <div class="qa__box">
          ...
          <div class="qa__text__box">
            ...
            <!-- 質問者のみにベストアンサーを選択を表示 -->
            <?php if($_SESSION['id'] == $question['member_id']): ?>
            <form class="qa__best__box" method="post" action="">
              <input type="hidden" name="id" value="<?php echo $answer['id']; ?>">
              <div class="send__btn__box"><input class="send__btn" type="submit" value="ベストアンサーに選ぶ"></div>
            </form>
            <?php endif; ?>
          <?php endforeach; ?>
